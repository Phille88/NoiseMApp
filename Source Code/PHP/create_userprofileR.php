...
if (isset($_POST['googleID']) && isset($_POST['firstName']) && isset($_POST['lastName']) && isset($_POST['email']) && strlen($_POST['googleID']) == 21 && is_numeric($_POST['googleID']) && isset($_POST['requestKey']) && $_POST["requestKey"] == REQUESTKEY) {
    ...
    $duplicatesql = "SELECT * FROM userprofilesr where (googleID_1 = '$googleID_1' AND googleID_2 = '$googleID_2')";
	...
	if (mysql_num_rows($duplicates) == false) {
		...
		if (isset($_POST['pictureURL'])){
			...
			$result = mysql_query("INSERT INTO userprofilesr(googleID_1, googleID_2,firstName, lastName, email, pictureURL, createdAt, updatedAt) VALUES('$googleID_1', '$googleID_2', '$firstName', '$lastName', '$email', '$pictureURL',now(),now())");
		}
		 else {
			$result = mysql_query("INSERT INTO userprofilesr(googleID_1, googleID_2,firstName, lastName, email, createdAt, updatedAt) VALUES('$googleID_1', '$googleID_2', '$firstName', '$lastName', '$email',now(),now())");
		}
		...
		$result = mysql_query("SELECT userID FROM userprofilesr WHERE (googleID_1 = '$googleID_1' AND googleID_2 = '$googleID_2')");
